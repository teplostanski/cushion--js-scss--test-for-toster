export class CustomSelect {
  static EL = 'custom-select'
  static EL_SHOW = 'custom-select_show'
  static EL_OPTION = 'custom-select__option'
  static EL_OPTION_SELECTED = 'custom-select__option_selected'
  ...
  static template(params) {
    ...
    options.forEach((option, index) => {
      ...
      items.push(`<li class="custom-select__option${selectedClass}" data-select="option"
        data-value="${option[0]}" data-index="${index}">${option[1]}</li>`)
    })
    return `<button type="button" class="custom-select__toggle" name="${name}"
      value="${selectedValue}" data-select="toggle" data-index="${selectedIndex}">
      ${selectedContent}</button><div class="custom-select__dropdown">
      <ul class="custom-select__options">${items.join('')}</ul></div>`
  }
  ...
  _updateOption(el) {
    ...
    this._el.dispatchEvent(new CustomEvent('custom-select.select.change'))
